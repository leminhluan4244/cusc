...
class PermissionAuth
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
        if (Auth::guard($guard)->check()) {
            ...
            if(!self::permission(Auth::user()->id)){
                // ---Return kết quả lỗi
    ...
    public static function permission($user_id){
        ...
        //Lấy ra phân quyền người đăng nhập hệ thống
        $accountCode = $user_id;
        $roles = UsersModel
            ::join('users_has_roles','users_has_roles.u_id','users.id')
            ->where('users.id',$accountCode)
            ->where('users.active',1)
            ->first();
        // Kiểm tra xem được phép đăng nhập link này không
        if(isset($roles)) {
            $result = PermissionModel::checkedRoute($roles->r_id,Route::getFacadeRoot()->current()->uri());
            if(isset($result)){
                return true;
            }
            return false;
