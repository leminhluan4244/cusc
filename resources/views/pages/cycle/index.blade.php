                ...
                @if(\App\Http\Controllers\RouteViewController::routeView('cycle/create'))
                    <div class=" float-md-right">
                        <button class="btn btn-info round  px-2 box-shadow-3" data-toggle="modal"
                                data-target="#addCycleModel" id="add" type="button"><i
                                    class="ft-plus icon-left"></i><span>Thêm loại chu kỳ</span></button>
                    </div>
                @endif
            </div>
        </div>
                                ...
                                <div class="card-body cleartfix">
                                    <div class="media align-items-stretch">
                                        <div class="media-body">
                                            <h5 class="text-bold-600 danger">{{$value->cy_name}}</h5>
                                            <span>Độ dài: {{$value->cy_long}}</span>
                                        </div>
                                        <div class="align-self-center">
                                            <i class="ft-settings white background-round bg-danger font-size-large"
                                               onclick="getControl('{{$value->cy_id}}','{{$value->cy_name}}','{{$value->cy_long}}')"></i>
                                        </div>
                                    </div>
                                </div>
                            </div>
